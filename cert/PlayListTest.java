...
public class PlayListTest {
    ...
    public void testMethods() {
		...
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "add", void.class, AudioFile.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "remove", void.class, AudioFile.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "size", int.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "currentAudioFile", AudioFile.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "nextSong", void.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "loadFromM3U", void.class, String.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "saveAsM3U", void.class, String.class);		
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "getList", List.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "getCurrent", int.class);
		Utils.checkMethod(PlayList.class, Modifier.PUBLIC, "setCurrent", void.class, int.class);
		
		// Only the following methods are allowed to be public on this class to fulfill the requirements
		Utils.checkAllowedAccessableMethods(PlayList.class, "add", "remove", "size", "saveAsM3U", "loadFromM3U", "nextSong", 
				"currentAudioFile", "getCurrent", "setCurrent", "getList");
	}
    ...
    public void testSequentialMode() {
    	...
    	for (int i = 0; i < pl.size(); i++) {
    		plCompare.add(pl.getList().get(i));
    	}
    ...
    @Test
    public void testLoadListWithWhitespaces() throws IOException {
        String pathname = "list-whitespaces.m3u";

        try {
            FileWriter writer = new FileWriter(new File(pathname));

            writer.write("\n");
            writer.write("audiofiles/Rock 812.mp3\n");
            writer.write("# ...\n");
            writer.write("\n");
            writer.write("audiofiles/wellenmeister - tranquility.wav\n");
            writer.write("\n");
            writer.close();
        } catch (Exception e) {
            fail("Could not create file 'list-withspaces.m3u'");
        }

        PlayList pl = new PlayList();
        pl.loadFromM3U(pathname);
        assertEquals("Size should be at 2", 2, pl.size());
        assertEquals("Load PlayList from M3U file yields wrong result",
            "[Eisbach - Rock 812 - The Sea, the Sky - 05:31, "
            + "wellenmeister - tranquility - 02:21]",
        pl.getList().toString());
    }
